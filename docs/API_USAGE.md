...
The observation record must contain every descriptor in `S(I)`. The selected policy defines the valuation functions and `Agg_C`; callers cannot replace them. The endpoint executes Equation 6 and returns both the aggregated result and each term used in the calculation.
