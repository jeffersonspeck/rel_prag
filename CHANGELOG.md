...
| --- | --- | --- |
| Section 4.1 and Equation 3, `C=<g,t,e,r,n>` | Context fields, roles, and norms must contain meaningful values. Empty contexts are rejected. | `test_context_and_provenance_cannot_be_empty` |
...
| Equations 2-4, `W(A,C)` and `w_i in [0,1]` | Policies enforce unit-interval weights, reject all-zero vectors, and must cover the complete descriptor schema. | `test_each_policy_has_provenance_and_matches_ontology` |
...
| Equations 8 and 17, descriptor-specific `s_i` | Every policy must define one comparison function for every descriptor. Comparator definitions are returned with similarity contributions. | `test_each_policy_has_provenance_and_matches_ontology`, `test_binary_similarity_is_not_numerical_identity` |
| Equation 18, thresholded similarity | Operational continuity can only use normalized scores. An unnormalized weighted sum is rejected before threshold evaluation. | `test_thresholded_weighted_sum_requires_normalized_weights` |
| Equation 23, `<I,S(I),A,C,W,Pi_W,v,s,Agg_C>` | Valuations, comparators, relevance aggregation, similarity aggregation, interaction rules, and threshold are versioned with the policy instead of being supplied ad hoc by callers. | `test_callers_cannot_replace_the_context_bound_aggregator` |
| Equations 18 and 23, traceability | Audit events preserve the complete request, policy configuration, and response needed to reproduce a contextual decision. | `test_audit_record_preserves_request_policy_and_response` |
| Equations 9-13, ship schema, contexts, and illustrative vectors | Policies reproduce `C_nav`, `C_hist`, the sailor and historian agents, both weight vectors, and the published scores of approximately `0.92` and `0.79`. | `test_policies_use_the_exact_weight_vectors_from_the_article`, `test_two_systems_consume_same_entity_with_distinct_policies` |
| Section 7.3, computational considerations | Contribution sorting was removed so the unary and single-pair binary kernels remain linear in descriptor count under the article's constant-time-comparator assumption. | Formal-order contribution assertions |
